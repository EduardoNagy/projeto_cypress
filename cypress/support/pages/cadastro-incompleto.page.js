...
Cypress.Commands.add('preencherParcialCadastro', () => {
...
Cypress.Commands.add('validarRegistroDados', () => {
    cy.get(BTN_SUBMIT_ACCOUNT).should('be.visible')
    cy.get(BTN_SUBMIT_ACCOUNT).click()
})    
    
    
Cypress.Commands.add('validarAlertDadosPessoais', () => {
    cy.get('p.pull-right.required').should('contain', 'Required field')
    
    cy.screenshot(`${Cypress.spec.name}/3-alerta dados pessoais não preenchidos`)
})